...
const App = () => {
  ...
  const handleProject1Click = async () => {
    try {
      ...
      handleShowModal(`Smart Battery Info: ${JSON.stringify(response.data, null, 2)}`);
    } catch (error) {
      console.error('Error fetching smart battery data:', error);
      alert('Error fetching smart battery data');
    }
  };

  const handleProject2Click = async () => {
    try {
      const response = await axios.get('https://jsonplaceholder.typicode.com/posts/2');
      handleShowModal(`Battery Swapping Info: ${JSON.stringify(response.data, null, 2)}`);
   } catch (error) {
      console.error('Error fetching battery swapping data:', error);
      alert('Error fetching battery swapping data');
    }
  };

  const handleProject3Click = async () => {
    try {
      const response = await axios.get('https://api.data.gov/ed/collegescorecard/v1/schools?api_key=DEMO_KEY');
      handleShowModal(`Fuel Replacement Info: ${JSON.stringify(response.data, null, 2)}`);
  } catch (error) {
      console.error('Error fetching fuel replacement data:', error);
      alert('Error fetching fuel replacement data');
    }
  };
